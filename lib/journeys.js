...
const createJourney = (date, formattedDate) => (j) => {
	...
	for (let section of sections) {
		...
		leg.departure = moment.tz(date.format('DD.MM.YYYY') + ' ' + section.departureTime, 'DD.MM.YYYY HH:mm', leg.origin.timezone).toISOString()
		...
		for (let s of section.trainStops) {
			...
			stopover.arrival = moment.tz(date.format('DD.MM.YYYY') + ' ' + s.arrival, 'DD.MM.YYYY HH:mm', stopover.stop.timezone).toISOString()
			lastTime = arrivalTime
			const departureTime = +moment.duration(s.departure)
			if (departureTime < lastTime) date.add(1, 'days')
			stopover.departure = moment.tz(date.format('DD.MM.YYYY') + ' ' + s.departure, 'DD.MM.YYYY HH:mm', stopover.stop.timezone).toISOString()
			lastTime = departureTime
		...
		if (arrivalTime < lastTime) date.add(1, 'days')
		leg.arrival = moment.tz(date.format('DD.MM.YYYY') + ' ' + section.arrivalTime, 'DD.MM.YYYY HH:mm', leg.destination.timezone).toISOString()
		lastTime = arrivalTime
