...
const createStopovers = (sts, date) => {
	...
	for (let st of sts) {
		...
		stopover.arrival = moment.tz(date.format('DD.MM.YYYY') + ' ' + st.arrival, 'DD.MM.YYYY HH:mm', stopover.stop.timezone).toISOString()
		lastTime = arrivalTime
		const departureTime = +moment.duration(st.departure)
		if (departureTime < lastTime) date.add(1, 'days')
		stopover.departure = moment.tz(date.format('DD.MM.YYYY') + ' ' + st.departure, 'DD.MM.YYYY HH:mm', stopover.stop.timezone).toISOString()
