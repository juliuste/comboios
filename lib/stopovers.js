...
const createStopovers = (sts, station, date, formattedDate) => {
	...
	for (let st of sts) {
		...
		if (st.arrivalTime) {
			...
			stopover.arrival = moment.tz(date.format('DD.MM.YYYY') + ' ' + st.arrivalTime, 'DD.MM.YYYY HH:mm', timezone).toISOString()
			lastTime = arrivalTime
		}
		if (st.departureTime) {
			const departureTime = +moment.duration(st.departureTime)
			if (departureTime < lastTime) date.add(1, 'days')
			stopover.departure = moment.tz(date.format('DD.MM.YYYY') + ' ' + st.departureTime, 'DD.MM.YYYY HH:mm', timezone).toISOString()
